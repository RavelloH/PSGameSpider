...
const generateLineChart = (data, d3n) => {
    ...
    let svg = d3n.createSVG(width, height).style('background-color', '#1e1e1e'); // Set background color
    ...
    let yAxis = (g) =>
        g.attr('transform', `translate(${margin.left},0)`).call(d3n.d3.axisLeft(y).ticks(5));

    svg.append('g').call(xAxis).selectAll('text').style('fill', '#ffffff'); // Set axis text color

    svg.append('g').call(yAxis).selectAll('text').style('fill', '#ffffff'); // Set axis text color
    ...
    svg.selectAll('path').attr('stroke', '#ffffff'); // Modify all line colors to white
